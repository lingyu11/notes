...
> 记忆化搜索：`@cache` 装饰器用于自动缓存函数的计算结果，当用相同参数多次调用函数时，`@cache` 会直接返回之前计算过的结果，而不是重新计算，提高效率。`@cache` 底层用的是哈希表（字典），以函数参数为 key，返回值为 value。每次函数调用时，先查缓存。如果有，直接返回；没有就计算并存入缓存。

> 动态规划的时间复杂度 = 状态个数 × 单个状态的计算时间。

### §1.1 爬楼梯

1. [爬楼梯](https://leetcode.cn/problems/climbing-stairs/description/)：<a id="climbing-stairs"></a>

    **关键思路：dfs(i) = dfs(i − 1) + dfs(i − 2), dfs(i) 表示从 0 爬到 i 的方案数：可以从台阶 i - 2 跳 2 步或从台阶 i - 1 跳 1 步**
...
1. [打家劫舍](https://leetcode.cn/problems/house-robber/description/)：<a id="house-robber"></a>

    **关键思路：dfs(i) = max(dfs(i - 1), dfs(i - 2) + nums[i]), 要么最后一个元素不选变成 dfs(i - 1)，要么选最后一个元素，变成dfs(i - 2) + nums[i] (因为所选元素不能相邻)**
